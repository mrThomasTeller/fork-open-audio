...
export default function Home() {
  const [apiKeyInput, setApiKey] = useLocalStorage(
    "open-audio-openai-api-key",
    ""
  );
  ...
  const [filename, setFilename] = useState("speech");
  ...
  const handleDownload = (audioUrl, index) => {
    saveAs(audioUrl, `${filename}-${index}.mp3`); // This will save the file as "speech.mp3"
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const chunks = splitText(inputText, maxLength);
        ...
        >
          ...
          >
            ...
            >
              ...
              <FormControl isRequired>
                <FormLabel htmlFor="input-text">Filename</FormLabel>
                <Input
                  id="filename"
                  placeholder="Enter the text you want to convert to speech"
                  value={filename}
              ...
              {audioUrls.map((audioUrl, index) => (
                <>
                  ...
                  <Button onClick={() => handleDownload(audioUrl, index)}>
                    Download MP3
                  </Button>
                </>
              ))}
